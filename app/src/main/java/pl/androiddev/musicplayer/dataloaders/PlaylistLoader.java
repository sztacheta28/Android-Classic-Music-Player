package pl.androiddev.musicplayer.dataloaders;
...
public class PlaylistLoader {
    ...
    public static List<Playlist> getPlaylists(Context context, boolean defaultIncluded) {
        ...
        if (mCursor != null && mCursor.moveToFirst()) {
            do {
                ...
                final int songCount = PlayerUtils.getSongCountForPlaylist(context, id);
    ...
    private static void makeDefaultPlaylists(Context context) {
        ...
        /* Last added list */
        final Playlist lastAdded = new Playlist(PlayerUtils.PlaylistType.LastAdded.mId,
                resources.getString(PlayerUtils.PlaylistType.LastAdded.mTitleId), -1);
        mPlaylistList.add(lastAdded);

        /* Recently Played */
        final Playlist recentlyPlayed = new Playlist(PlayerUtils.PlaylistType.RecentlyPlayed.mId,
                resources.getString(PlayerUtils.PlaylistType.RecentlyPlayed.mTitleId), -1);
        mPlaylistList.add(recentlyPlayed);

        /* Top Tracks */
        final Playlist topTracks = new Playlist(PlayerUtils.PlaylistType.TopTracks.mId,
                resources.getString(PlayerUtils.PlaylistType.TopTracks.mTitleId), -1);
